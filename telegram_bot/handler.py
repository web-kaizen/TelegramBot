...
dialogue_list = DialogueList.DialogueList(method="GET", need_execute_local=True, token=login_response["result"]["access_token"])
dialogue_list_response = dialogue_list.get_response()
print(f"DialogueList: {dialogue_list_response}")

time.sleep(1)

dialogue_details = DialogueDetail.DialogueDetail(method="GET", dialogue_id=2, need_execute_local=True, token=login_response["result"]["access_token"])
dialogue_details_response = dialogue_details.get_response()
print(f"DialogueDetail: {dialogue_details_response}")

time.sleep(1)

email_check = EmailVerificationCheck.EmailVerificationCheck(need_execute_local=True, token=login_response["result"]["access_token"])
email_check_response = email_check.get_response()
print(f"EmailCheck: {email_check_response}")

time.sleep(1)

email_resend = EmailVerificationResend.EmailVerificationResend(need_execute_local=True, token=login_response["result"]["access_token"])
email_resend_response = email_resend.get_response()
print(f"EmailResend: {email_resend_response}")

time.sleep(1)

email_verify = EmailVerificationVerify.EmailVerificationVerify(need_execute_local=True, token=login_response["result"]["access_token"])
email_verify_response = email_verify.get_response()
print(f"EmailVerify: {email_verify_response}")

time.sleep(1)

logout = Logout.Logout(need_execute_local=True, token=login_response["result"]["access_token"])
